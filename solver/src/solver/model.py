...
class WordleModel:
    ...
    def __init__(self, *, strategy: Strategy = Strategy.ENTROPY) -> None:
    ...
    def expected_entropy(guess: str, candidates: Sequence[str]) -> float:
        ...
        entropy = 0.0
        for bucket in WordleModel.partition(candidates, guess).values():
            probability = len(bucket) / total
            entropy -= probability * math.log2(probability)
        return entropy

    @staticmethod
    def minimax_worst_case(guess: str, candidates: Sequence[str]) -> float:
        """Return the size of the largest remaining bucket after playing `guess`."""
        if not candidates:
            return 0.0
        return float(
            max(len(bucket) for bucket in WordleModel.partition(candidates, guess).values())
        )
